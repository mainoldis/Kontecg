...
namespace Kontecg.Tests.Authorization
{
    public class AuthorizationInterceptor_Tests : TestBaseWithLocalIocManager
    {
        ...
        public AuthorizationInterceptor_Tests()
        {
            ...
            LocalIocManager.Register<IMultiCompanyConfig, MultiCompanyConfig>();
            LocalIocManager.Register<AuthorizationInterceptor>(DependencyLifeStyle.Transient);
            LocalIocManager.Register<KontecgAsyncDeterminationInterceptor<AuthorizationInterceptor>>(DependencyLifeStyle.Transient);
            LocalIocManager.Register<IAuthorizationHelper, AuthorizationHelper>(DependencyLifeStyle.Transient);
            LocalIocManager.IocContainer.Register(
                Component.For<MyTestClassToBeAuthorized_Sync>().Interceptors<KontecgAsyncDeterminationInterceptor<AuthorizationInterceptor>>().LifestyleTransient(),
                Component.For<MyTestClassToBeAuthorized_Async>().Interceptors<KontecgAsyncDeterminationInterceptor<AuthorizationInterceptor>>().LifestyleTransient(),
                Component.For<MyTestClassToBeAllowProtected_Async>().Interceptors<KontecgAsyncDeterminationInterceptor<AuthorizationInterceptor>>().LifestyleTransient(),
                Component.For<MyTestClassToBeAllowProtected_Sync>().Interceptors<KontecgAsyncDeterminationInterceptor<AuthorizationInterceptor>>().LifestyleTransient()
                );
            ...
            LocalIocManager.IocContainer.Register(Component.For<IKontecgSession>().Instance(session));
        ...
        public void Test_Authorization_Sync()
        {
            ...
            Assert.Throws<KontecgAuthorizationException>(() => _syncObj.MethodWithPermission3());
            _syncObj.Called_MethodWithPermission3.ShouldBe(false);

            Assert.Throws<KontecgAuthorizationException>(() => _syncObj.MethodWithPermission1AndPermission3WithRequireAll());
        ...
        public async Task Test_Authorization_Async()
        {
            ...
            await Assert.ThrowsAsync<KontecgAuthorizationException>(async () => await _asyncObj.MethodWithPermission3Async());
            _asyncObj.Called_MethodWithPermission3.ShouldBe(false);

            await Assert.ThrowsAsync<KontecgAuthorizationException>(async () => await _asyncObj.MethodWithPermission1AndPermission3WithRequireAllAsync());
        ...
        public void Test_Authorization_For_Protected_Sync()
        {
            ...
            Assert.Throws<KontecgAuthorizationException>(() => _syncObjForProtectedMethod.MethodWithPermissionForProtected());
        ...
        public async Task Test_Authorization_For_Protected_Async()
        {
            ...
            await Assert.ThrowsAsync<KontecgAuthorizationException>(async () => await _asyncObjForProtectedMethod.MethodWithPermissionForProtectedAsync());
            _asyncObjForProtectedMethod.Called_AuthorizedProtectedMethod.ShouldBe(false);
        }

        private void EmptySession()
        {
            LocalIocManager.Resolve<IKontecgSession>().CompanyId.Returns((int?) null);
            LocalIocManager.Resolve<IKontecgSession>().UserId.Returns((int?) null);
        }
        ...
        public class MyTestClassToBeAuthorized_Sync
        {
            ...
            [KontecgAuthorize("Permission1", "Permission2")]
            ...
            [KontecgAuthorize("Permission1", "Permission3")]
            ...
            [KontecgAuthorize("Permission1", "Permission3", RequireAllPermissions = true)]
