...
namespace Kontecg.Tests.Runtime.Caching.Memory
{
    public class MemoryCacheManager_Tests : TestBaseWithLocalIocManager
    {
        ...
        public MemoryCacheManager_Tests()
        {
            ...
            LocalIocManager.Register<ICacheManager, KontecgMemoryCacheManager>();
            LocalIocManager.Register<MyClientPropertyInjects>(DependencyLifeStyle.Transient);
            LocalIocManager.IocContainer.Register(Component.For<IKontecgStartupConfiguration>().Instance(Substitute.For<IKontecgStartupConfiguration>()));
