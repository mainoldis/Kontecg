...
using Kontecg.EntityFrameworkCore.Tests.Ef;
using Kontecg.TestBase;
using Kontecg.Timing;

namespace Kontecg.EntityFrameworkCore.Tests
{
    public abstract class EntityFrameworkCoreModuleTestBase : KontecgIntegratedTestBase<EntityFrameworkCoreTestModule>
    {
        ...
        private void CreateInitialData()
        {
            UsingDbContext(
                context =>
                {
                    ...
                    var post4 = new Post { Blog = blog1, Title = "test-post-4-title", Body = "test-post-4-body", CompanyId = 42 };
            ...
            using (var context = LocalIocManager.Resolve<SupportDbContext>())
            {
                context.Tickets.AddRange(
                    new Ticket { EmailAddress = "john@aspnetboilerplate.com", Message = "an active message", CompanyId = 1 },
                    new Ticket { EmailAddress = "david@aspnetboilerplate.com", Message = "an inactive message", IsActive = false, CompanyId = 1 },
                    new Ticket { EmailAddress = "smith@aspnetboilerplate.com", Message = "an active message of company 42", CompanyId = 42 }
                );
