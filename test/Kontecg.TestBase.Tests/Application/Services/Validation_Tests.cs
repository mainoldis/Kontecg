...
{
    public class Validation_Tests : KontecgIntegratedTestBase<KontecgKernelModule>
    {
        ...
        {
            Assert.Throws<KontecgValidationException>(() => _myAppService.MyMethod(new MyMethodInput())); //MyStringValue is not supplied!
            Assert.Throws<KontecgValidationException>(() => _myAppService.MyMethod(new MyMethodInput { MyStringValue = "a" })); //MyStringValue's min length should be 3!
        }
        ...
        {
            Assert.Throws<KontecgValidationException>(() =>
