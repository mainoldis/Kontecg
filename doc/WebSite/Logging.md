...
Kontecg platform uses Castle Windsor's [logging
...
popular logging libraries for .NET. The Kontecg platform
...
Kontecg platform provides **base classes** for MVC controllers, Web
...
base controller that inherits **KontecgController.** This way, it can directly
...
from the Kontecg platform [templates](/Templates).
...
log4net.config file:
    ...
    public class MvcApplication : KontecgWebApplication
    {
...
#### Kontecg.Castle.Log4Net Package

kontecg uses the Castle Logging Facility for logging and it does not directly
...
[**Kontecg.Castle.Log4Net**](http://nuget.org/packages/Kontecg.Castle.Log4Net),
...
have to do is to change the code in the application start method like this:
    ...
    public class MvcApplication : KontecgWebApplication
    {
        protected override void Application_Start(object sender, EventArgs e)
        {
            IocManager.Instance.IocContainer.AddFacility<LoggingFacility>(f => f.UseKontecgLog4Net().WithConfig("log4net.config"));
            base.Application_Start(sender, e);
        }
    }

The only difference is that we used the "**UseKontecgLog4Net()**" method
(defined in Kontecg.Castle.Logging.Log4Net namespace) instead of
"UseLog4Net()". When we use Kontecg.Castle.Log4Net package, you **do not
need** to use the
...
Kontecg platform defines a simple JavaScript logging API for the client
side. It logs to the browser's console as default. Here's some JavaScript code to
write logs:

    kontecg.log.warn('a sample log message...');
