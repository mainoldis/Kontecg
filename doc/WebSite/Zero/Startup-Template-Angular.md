...
The easiest way of starting a new project using kontecg with **ASP.NET Core** and **Angular** is to create a
...
In this template, **multi-company is enabled by default**. If you don't
...
login as a company, you need to first switch to that company on the login page. By default, there
is a company named "Default". Once you login successfully, you will
see a dashboard:
...
The **API will work as host users by default**. You can send the **Kontecg.CompanyId**
header value to work with a specified company. It's an integer value and by default is
...
the company databases and runs the migrations for those databases. It skips a
company if it does not have a dedicated database or if the database has already
been migrated by another company (for databases shared between multiple companys).
...
since they test your code with all of Kontecg platform's infrastructure
