...
The easiest way of starting a new project using kontecg with **ASP.NET MVC 5.x (and optionally AngularJS frontend)** is to create a template on the [download page](/Templates).
...
because the Kontecg.Web.Resources NuGet package comes with d.ts and it requires
Typescript 2.0+.
...
**password**. **tenancyName** should also be sent for **company** users.
