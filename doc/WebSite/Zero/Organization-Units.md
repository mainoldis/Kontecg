...
-   **CompanyId**: Company's Id of this OU. Can be null for host OUs.
-   **ParentId**: Parent OU's Id. Can be null if this is a root OU.
-   **Code**: A hierarchical string code that is unique for a company.
...
Since an OU can have a parent, all OUs of a company are in a **tree**
...
-   It must be **unique** for a [company](/Pages/Documents/Multi-Company).
...
The OrganizationUnitManager is designed to work for a **single company** at a
time. It works for the **current company** by default.
...
```csharp
public class Product : Entity, IMustHaveCompany, IMustHaveOrganizationUnit
{
...
**Please note**; The product entity must have a **CompanyId** (which is a property
of IMustHaveCompany) to distinguish it from products of different companys in a
multi-company application (see the [Multi-Company
document](/Pages/Documents/Multi-Company#data-filters) for more info). If your
application is not multi-company, you don't need this interface and
property.
...
public class ProductManager : IDomainService
{
    ...
    {
		var user = await _userManager.GetUserByIdAsync(KontecgSession.UserId.Value);
...
-   **MaxUserMembershipCount**: Maximum allowed membership count for a
    ...
    *KontecgZeroSettingNames.OrganizationUnits.MaxUserMembershipCount*.
