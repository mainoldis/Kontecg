...
In this article, we will see a SaaS (multi-company) application developed using the following frameworks:

- Kontecg platform as application framework.
...
Kontecg platform provides templates to make a project startup easier. We create the startup template from https://aspnetboilerplate.com/Templates

I selected **ASP.NET Core 2.x**, **Angular** and checked **"Include login, register, user, role and company management pages"**. It creates a ready and working solution for us including a login page, navigation and a bootstrap based layout. After download and open the solution with Visual Studio 2017+, we see a layered solution structure including a unit test project.
...
Event Cloud is a free SaaS (multi-company) application. We can create a company which has its own events, users, roles... There are some simple business rules applied while creating, canceling and registering to an event.

So, let's start to investigate the source code.

### Entities

Entities are parts of our domain layer and located under **EventCloud.Core** project. **Kontecg platform** startup template comes with **Company**, **User**, **Role** ... entities which are common for most applications. We can customize them based on our needs. Surely, we can add our application specific entities.

The fundamental entity of event cloud project is the `Event` entity.

```c#
[Table("AppEvents")]
public class Event : FullAuditedEntity<Guid>, IMustHaveCompany
{
    ...
    public static Event Create(int companyId, string title, DateTime date, string description = null, int maxRegistrationCount = 0)
    {
        var @event = new Event
        {
            Id = Guid.NewGuid(),
    ...
    private void SetDate(DateTime date)
    {
        ...
        if (date <= Clock.Now.AddHours(3)) //3 can be configurable per company
        {
...
Event implements, `IMustHaveCompany` interface. This is an interface of **Kontecg platform (kontecg)** framework and ensures that this entity is per company. This is needed for multi-company. Thus, different companys will have different events and can not see each other's events. **kontecg** automatically filters entities of current company.

Event class inherits from `FullAuditedEntity` which contains creation, modification and deletion audit columns. `FullAuditedEntity` also implements `ISoftDelete`, so events can not be deleted from database. They are marked as deleted when you delete it. **kontecg** automatically filters (hides) deleted entities when you query database.
...
public class EventRegistration : CreationAuditedEntity, IMustHaveCompany
{
...
We handle these events and notify related users about these changes. Also, I handle `EntityCreatedEventDate<Event>` (which is a pre-defined **kontecg** event and triggered automatically).
        ...
        ITransientDependency
{
    ...
    {
        //TODO: Send email to all company users as a notification

        var users = _userManager
            .Users
            .Where(u => u.CompanyId == eventData.Entity.CompanyId)
            .ToList();
...
public class EventAppService : EventCloudAppServiceBase, IEventAppService
{
    ...
    {
        var @event = Event.Create(KontecgSession.GetCompanyId(), input.Title, input.Date, input.Description, input.MaxRegistrationCount);
            ...
            (result: boolean) => {
                if (result) {
                    this._eventService.cancelAsync(event)
                        .subscribe(() => {
                            kontecg.notify.info('Event is deleted');
                            this.refresh();
                        });
                }
            }
...
export class EventDetailComponent extends AppComponentBase implements OnInit {
    ...
    registerToEvent(): void {
        ...
        this._eventService.registerAsync(input)
            .subscribe((result: EventRegisterOutput) => {
                kontecg.notify.success('Successfully registered to event. Your registration id: ' + result.registrationId + ".");
                this.loadEvent();
            });
    };
    ...
    cancelRegistrationFromEvent(): void {
        ...
        this._eventService.cancelRegistrationAsync(input)
            .subscribe(() => {
                kontecg.notify.info('Canceled your registration.');
                this.loadEvent();
            });
    };
    ...
    cancelEvent(): void {
        ...
        this._eventService.cancelAsync(input)
            .subscribe(() => {
                kontecg.notify.info('Canceled the event.');
                this.backToEventsPage();
            });
    };

    isRegistered(): boolean {
        return _.some(this.event.registrations, { userId: kontecg.session.userId });
    };

    isEventCreator(): boolean {
        return this.event.creatorUserId === kontecg.session.userId;
    };
...
Top menu is automatically created by **kontecg template**. We define menu items in `sidebar-nav.component.ts` class:
...
export class SideBarNavComponent extends AppComponentBase {
    ...
    menuItems: MenuItem[] = [
        ...
        new MenuItem(this.l("Companys"), "Pages.Companys", "business", "/app/companys"),
        ...
        RouterModule.forChild([
            {
                ...
                children: [
                    ...
                    { path: 'companys', component: CompanysComponent, data: { permission: 'Pages.Companys' }, canActivate: [AppRouteGuard] },
...
**Kontecg platform** provides tools to make unit and integration tests easier. You can find all test code from source code of the project. Here, I will briefly explain basic tests. Solution includes `EventAppService_Tests` class which tests the `EventAppService`. See 2 tests from this class:
...
public class EventAppService_Tests : EventCloudTestBase
{
    ...
    public async Task Should_Register_To_Events()
    {
        ...
        UsingDbContext(context =>
        {
            var currentUserId = KontecgSession.GetUserId();
...
With such tests, we tested everything starting from application service including all aspects of **Kontecg platform** (like validation, unit of work and so on).
