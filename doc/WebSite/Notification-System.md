...
system. Kontecg platform provides a **pub/sub** (publish/subscribe) based **real-time**
notification system.
...
notifications. Examples:
    ...
    public class MyService : ITransientDependency
    {
        ...
        public async Task Subscribe_SentFriendshipRequest(int? companyId, long userId)
        {
            await _notificationSubscriptionManager.SubscribeAsync(new UserIdentifier(companyId, userId), "SentFriendshipRequest");    
        }

        //Subscribe to an entity notification
        public async Task Subscribe_CommentPhoto(int? companyId, long userId, Guid photoId)
        {
            await _notificationSubscriptionManager.SubscribeAsync(new UserIdentifier(companyId, userId), "CommentPhoto", new EntityIdentifier(typeof(Photo), photoId));   
        }
    }
...
**INotificationPublisher** is used to publish notifications. Examples:
    ...
    public class MyService : ITransientDependency
    {
        ...
        //Send a general notification to all subscribed users in current company (company in the session)
...
Kontecg platform supports multiple **IRealTimeNotifiers** to notify
...
When a real-time notification is received via SignalR, Kontecg platform triggers
a **global event** on the client-side. You can register it like this to get
notifications:

    kontecg.event.on('kontecg.notifications.received', function (userNotification) {
        console.log(userNotification);
    });

The **kontecg.notifications.received** event is triggered for each received real-
...

    {
        ...
        "notification": {
            ...
            "data": {
                ...
                "type": "Kontecg.Notifications.LocalizableMessageNotificationData",
                "properties": {
...
data to show notification information to the user. Example:
    ...
    kontecg.event.on('kontecg.notifications.received', function (userNotification) {
        if (userNotification.notification.data.type === 'Kontecg.Notifications.LocalizableMessageNotificationData') {
            var localizedText = kontecg.localization.localize(
        ...
        } else if (userNotification.notification.data.type === 'Kontecg.Notifications.MessageNotificationData') {
...
[kontecg.notify](/Pages/Documents/Javascript-API/Notification) api instead to show
...
notification is received:
    ...
    kontecg.event.on('kontecg.notifications.received', function (userNotification) {
        kontecg.notifications.showUiNotifyForUserNotification(userNotification);
    });
...
formatters like this:
    ...
    kontecg.notifications.messageFormatters['MyProject.MyNotificationDataType'] = function(userNotification) {
...
directly use
**kontecg.notifications.getFormattedMessageFromUserNotification(userNotification)**
