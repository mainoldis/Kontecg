...
Kontecg platform provides extensive support for the development of world-ready and localized applications.
...
**kontecg.localization** JavaScript API to get a list of all available
...
The Kontecg platform [templates](/Templates) use this system to show a
...
localization source. It is then **register**ed to Kontecg platform's
...
requested, Kontecg platform gets the text from the current language's XML
...
In this case, Kontecg platform uses .NET's built-in resource manager
...
Kontecg.AspNetCore package automatically adds ASP.NET Core's
...
-   **KontecgUserRequestCultureProvider** (kontecg's provider): If the user is known
    via **[IKontecgSession](Kontecg-Session.md)** and has explicitly selected a
...
-   **KontecgLocalizationHeaderRequestCultureProvider** (kontecg's provider):
...
-   **KontecgDefaultRequestCultureProvider** (kontecg's provider): If there is
    an default/application/company **setting value** for the language
    (named "Kontecg.Localization.DefaultLanguageName"), then use the
    setting's value.

The **UseRequestLocalization** middleware is automatically added when
you call the **app.UseKontecg()** method. However, it's suggested that you manually add
...
language. Example usage:
    ...
    {
        app.UseKontecg(options =>
        {
            options.UseKontecgRequestLocalization = false; //disable automatic adding of request localization
        });
    
        //...authentication middleware(s)
    
        app.UseKontecgRequestLocalization(); //manually add request localization
...
Most of time, you don't need to worry if you are using kontecg's
...
kontecg automatically determines the current language in every web request and
sets the **current thread's culture (and UI culture)**. This is how kontecg
determines it by default. kontecg will:

-   Try to get it from a special **query string** value, named
    "**Kontecg.Localization.CultureName**" by default.
-   If the user is known via [IKontecgSession](Kontecg-Session.md) and has explicitly
...
-   Try to get it from **default culture** setting (setting name is
    "Kontecg.Localization.DefaultLanguageName", which is a constant defined
    in Kontecg.Localization.LocalizationSettingNames.DefaultLanguage and can
...
in your module's PreInitialize method. Example:
    ...
    Configuration.Modules.KontecgWeb().Localization.CookieName = "YourCustomName";

kontecg overrides **Application\_PostAuthenticateRequest** (in global.asax)
...
kontecg has built-in `KontecgLocalizationController`, its `ChangeCulture` method changes the current language by writing cookies.
It also changes the user's default language setting if the current user exists.

    <a href="~/KontecgLocalization/ChangeCulture?cultureName=en-AU&returnUrl=Home"></a>
...
After creating a source and registering it to the Kontecg platform's
...
from **KontecgServiceBase**, there are shortcut **L** methods.
...
source name. It's done in the SimpleTaskSystemControllerBase as shown below:
    ...
    public abstract class SimpleTaskSystemControllerBase : KontecgController
    {
...
Note that it is derived from **KontecgController** and therefore, you can easily localize text with the **L** method.
...
sets the source name:
    ...
    public abstract class SimpleTaskSystemWebViewPageBase<TModel> : KontecgWebViewPage<TModel>
    {
...
solution from one of the Kontecg platform [templates](/Templates).

#### In JavaScript

Kontecg platform also makes it possible to use the same localization text in
JavaScript. First, you need to add the dynamic kontecg scripts to
the page:

    <script src="/KontecgScripts/GetScripts" type="text/javascript"></script>

Kontecg platform automatically generates the needed JavaScript code to
get localized text on the client side. You can then easily get a
localized text in JavaScript as shown below:

    var s1 = kontecg.localization.localize('NewTask', 'SimpleTaskSystem');
...
text:
    ...
    var source = kontecg.localization.getSource('SimpleTaskSystem');
...
The localization method can also get additional format arguments. Example:
    ...
    kontecg.localization.localize('RoleDeleteWarningMessage', 'MySource', 'Admin');
...
kontecg.localization.localize method without the source name.

    kontecg.localization.defaultSourceName = 'SimpleTaskSystem';
    var s1 = kontecg.localization.localize('NewTask');
...
to other languages. Kontecg platform allows for extending a localization
...
Kontecg platform also defines some localization sources. For
instance, the **Kontecg.Web** NuGet package defines a localization source named
"**KontecgWeb**" as embedded XML files:

<img src="images/KontecgWeb-localization-source.png" alt="KontecgWeb localization source files" class="img-thumbnail" />
...
To extend KontecgWeb source, we can define XML files. Assume that we only
...
We can then register it on the PreInitialize method of our module:
    ...
    Configuration.Localization.Sources.Extensions.Add(
    	new LocalizationSourceExtensionInfo("KontecgWeb",
    		new XmlEmbeddedFileLocalizationDictionaryProvider(
    			Assembly.GetExecutingAssembly(),
    			"MyCompany.MyProject.Localization.Sources"
    		)
...
Kontecg platform overrides (merges) the base localization source with our
...
-   XML/JSON files require string keys while getting localized texts
    ...
    **database based** and **per-company** localization source. See
