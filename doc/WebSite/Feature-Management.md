...
Kontecg platform provides a **feature system** to make it easier. You
can **define** features, **check** if a feature is **enabled** for a
company, and **integrate** the feature system to other Kontecg platform
...
**disabled** (for an edition or for a company).
...
-   **Default value**: A default value. This is used when we need the
    value of the feature and it's not available for current company.
...
-   **Scope**: A value in the FeatureScopes enum. It can be **Edition** (if
    this feature can be set only for edition level), **Company** (if this
    feature can be set only for company level) or **All** (if this
    feature can be set for editions and companys, where a company setting
...
Note that the Input type definitions are not used by Kontecg platform.
They can be used by applications to create inputs for features.
Kontecg platform just provides the infrastructure to make it easier.
...
Kontecg platform **does not** enforce this, but we recommend it.
The application should take care of it.

### Checking Features

We define a feature to check its value in the application to allow or
block some application features per company. There are different ways of
checking it.
...
the **current company** (current company is obtained from
[IKontecgSession](/Pages/Documents/Kontecg-Session)). If it's not enabled, an
**KontecgAuthorizationException** is thrown automatically.
...
This is used to simply check if a given feature is enabled or not. Example:
    ...
    public async Task<FileDto> GetReportToExcel(...)
    {
        ...
        {
            throw new KontecgAuthorizationException("You don't have this feature: ExportToExcel");
        }
...
Used to get the current value of a feature for value-type features. Example:
    ...
    {
        throw new KontecgAuthorizationException("You exceed task creation limit for this month, sorry :(");
    }

The FeatureChecker methods also have overrides to check features not only for the
current companyId, but for a **specified** companyId as well.

#### Client Side

On the client side (JavaScript), we can use the **kontecg.features** namespace
to get the current values of features.

##### isEnabled
```csharp
    var isEnabled = kontecg.features.isEnabled('SampleBooleanFeature');
```
##### getValue
```csharp
    var value = kontecg.features.getValue('SampleNumericFeature');
```

#### Ignore Feature Check For Host Users

If you enabled Multi-Company, then you can also ignore feature check for host users by configuring it in PreInitialize method of our module as shown below:
...
The Kontecg platform framework does not have a built-in edition system because
such a system requires a database (to store editions, edition features,
company-edition mappings and so on...). Therefore, the edition system is
