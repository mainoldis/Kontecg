...
namespace Kontecg.MailKit
{
    public static class EmailExtensions
    {
        ...
        public static MimeMessage ToMimeMessage(this MailMessage mail)
        {
            ...
            foreach (string field in mail.Headers.AllKeys)
            foreach (string value in mail.Headers.GetValues(field)!)
            {
                headers.Add(new Header(field, value));
            }

            MimeMessage message = new(headers);
            ...
            {
                TextPart text = new(mail.IsBodyHtml ? "html" : "plain");
            ...
            if (mail.AlternateViews.Count > 0)
            {
                ...
                {
                    MimePart part = GetMimePart(view);
                    ...
                    {
                        string type = part.ContentType.MediaType + "/" + part.ContentType.MediaSubtype;
        ...
        {
            string mimeType = item.ContentType.ToString();
            ...
            if (attachment != null)
            {
                string disposition = attachment.ContentDisposition.ToString();
