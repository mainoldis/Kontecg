...
{
    public class MayHaveCompanyDapperQueryFilter : IDapperQueryFilter
    {
        private readonly ICurrentUnitOfWorkProvider _currentUnitOfWorkProvider;

        public MayHaveCompanyDapperQueryFilter(ICurrentUnitOfWorkProvider currentUnitOfWorkProvider)
        {
        ...
        public string FilterName { get; } = KontecgDataFilters.MayHaveCompany;
        ...
        public IFieldPredicate ExecuteFilter<TEntity, TPrimaryKey>() where TEntity : class, IEntity<TPrimaryKey>
        {
            ...
            if (typeof(IMayHaveCompany).IsAssignableFrom(typeof(TEntity)) && IsEnabled)
            {
                predicate = Predicates.Field<TEntity>(f => (f as IMayHaveCompany).CompanyId, Operator.Eq, CompanyId);
            }
            ...
            Expression<Func<TEntity, bool>> predicate) where TEntity : class, IEntity<TPrimaryKey>
        {
            if (typeof(IMayHaveCompany).IsAssignableFrom(typeof(TEntity)) && IsEnabled)
            {
                PropertyInfo propType = typeof(TEntity).GetProperty(nameof(IMayHaveCompany.CompanyId));
                if (predicate == null)
                {
                    predicate = ExpressionUtils.MakePredicate<TEntity>(nameof(IMayHaveCompany.CompanyId), CompanyId,
                        propType.PropertyType);
                }
                else
                {
                    ParameterExpression paramExpr = predicate.Parameters[0];
                    MemberExpression memberExpr = Expression.Property(paramExpr, nameof(IMayHaveCompany.CompanyId));
