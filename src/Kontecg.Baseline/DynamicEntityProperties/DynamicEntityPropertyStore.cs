...
namespace Kontecg.DynamicEntityProperties
{
    public class DynamicEntityPropertyStore : IDynamicEntityPropertyStore, ITransientDependency
    {
        ...
        public DynamicEntityPropertyStore(IRepository<DynamicEntityProperty> dynamicEntityPropertyRepository,
        ...
        {
            return _dynamicEntityPropertyRepository.GetAllIncluding(e => e.DynamicProperty)
                .Where(x => x.EntityFullName == entityFullName).OrderBy(edp => edp.EntityFullName)
                .ThenBy(edp => edp.DynamicPropertyId).ToList();
        }
