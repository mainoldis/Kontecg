...
namespace Kontecg.DynamicEntityProperties
{
    public class DynamicEntityPropertyValueStore : IDynamicEntityPropertyValueStore, ITransientDependency
    {
        ...
        {
            List<DynamicEntityPropertyValue> list = _dynamicEntityPropertyValueRepository.GetAll().Where(val =>
                val.EntityId == entityId && val.DynamicEntityPropertyId == dynamicEntityPropertyId).ToList();

            foreach (DynamicEntityPropertyValue dynamicEntityPropertyValue in list)
            {
        ...
        {
            List<DynamicEntityPropertyValue> list = await _asyncQueryableExecuter.ToListAsync(
                _dynamicEntityPropertyValueRepository.GetAll().Where(
                    val =>
