...
namespace Kontecg.IdentityFramework
{
    public static class IdentityResultExtensions
    {
        ...
        public static string LocalizeErrors(this IdentityResult identityResult,
            ILocalizationManager localizationManager,
            string localizationSourceName = KontecgBaselineConsts.LocalizationSourceName)
        {
            if (identityResult.Succeeded)
            {
            ...
            ILocalizationSource localizationSource = localizationManager.GetSource(localizationSourceName);
            return identityResult.Errors.Select(err => LocalizeErrorMessage(err.Description, localizationSource))
                .JoinAsString(" ");
        }

        private static string LocalizeErrorMessage(string identityErrorMessage, ILocalizationSource localizationSource)
        {
            foreach (KeyValuePair<string, string> identityLocalization in IdentityLocalizations)
            {
                if (FormattedStringValueExtracter.IsMatch(identityErrorMessage, identityLocalization.Key,
                        out string[] values))
                {
