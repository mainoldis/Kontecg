...
namespace Kontecg.Webhooks
{
    ...
    public class WebhookEventStore : IWebhookEventStore, ITransientDependency
    {
        ...
        public virtual async Task<Guid> InsertAndGetIdAsync(WebhookEvent webhookEvent)
        {
            ...
            {
                using (_unitOfWorkManager.Current.SetCompanyId(webhookEvent.CompanyId))
                {
        ...
        public virtual async Task<WebhookEvent> GetAsync(int? companyId, Guid id)
        {
            return await _unitOfWorkManager.WithUnitOfWorkAsync(async () =>
            {
        ...
        public virtual WebhookEvent Get(int? companyId, Guid id)
        {
            return _unitOfWorkManager.WithUnitOfWork(() =>
            {
                using (_unitOfWorkManager.Current.SetCompanyId(companyId))
