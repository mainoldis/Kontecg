...
namespace Kontecg.Runtime.Caching.Redis
{
    ...
    /// </summary>
    public class KontecgRedisCache : CacheBase
    {
        private readonly IDatabase _database;
        private readonly IRedisCacheSerializer _serializer;

        protected IKontecgRedisCacheKeyNormalizer KeyNormalizer { get; }
        protected IMultiCompanyConfig MultiCompanyConfig { get; }

        /// <summary>
        /// Constructor.
        /// </summary>
        public KontecgRedisCache(
            string name,
            IKontecgRedisCacheDatabaseProvider redisCacheDatabaseProvider,
            IRedisCacheSerializer redisCacheSerializer,
            IKontecgRedisCacheKeyNormalizer keyNormalizer,
        ...
        protected virtual RedisKey NormalizeKey(string key)
        {
            return KeyNormalizer.NormalizeKey(
                new KontecgRedisCacheKeyNormalizeArgs(
                    key,
                    Name,
                    MultiCompanyConfig.IsEnabled
                )
            );
        }
        ...
        public override void Set(string key, object value, TimeSpan? slidingExpireTime = null, DateTimeOffset? absoluteExpireTime = null)
        {
            if (value == null)
            {
                throw new KontecgException("Can not insert null values to the cache!");
            }
        ...
        protected virtual Type GetSerializableType(object value)
        {
            ...
            //TODO: Normally, entities should not be stored in the cache, but currently Kontecg.Zero packages does it. It will be fixed in the future.
