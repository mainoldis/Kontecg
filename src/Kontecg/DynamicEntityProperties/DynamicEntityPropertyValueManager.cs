...
namespace Kontecg.DynamicEntityProperties
{
    public class DynamicEntityPropertyValueManager : IDynamicEntityPropertyValueManager, ITransientDependency
    {
        ...
        {
            DynamicEntityPropertyValue value = DynamicEntityPropertyValueStore.Get(id);
        ...
        {
            DynamicEntityPropertyValue value = await DynamicEntityPropertyValueStore.GetAsync(id);
        ...
        public virtual void Delete(long id)
        {
            DynamicEntityPropertyValue
                dynamicEntityPropertyValue = Get(id); //Get checks permission, no need to check it again
        ...
        {
            DynamicEntityPropertyValue
                dynamicEntityPropertyValue = await GetAsync(id); //Get checks permission, no need to check it again
            if (dynamicEntityPropertyValue == null)
            {
                return;
            }
        ...
        {
            DynamicEntityProperty dynamicEntityProperty = _dynamicEntityPropertyManager.Get(dynamicEntityPropertyId);
        ...
        public async Task<List<DynamicEntityPropertyValue>> GetValuesAsync(int dynamicEntityPropertyId, string entityId)
        {
            DynamicEntityProperty dynamicEntityProperty =
                await _dynamicEntityPropertyManager.GetAsync(dynamicEntityPropertyId);
        ...
        public List<DynamicEntityPropertyValue> GetValues(string entityFullName, string entityId)
        {
            return DynamicEntityPropertyValueStore.GetValues(entityFullName, entityId)
                ...
                {
                    DynamicEntityProperty dynamicEntityProperty =
                        _dynamicEntityPropertyManager.Get(value.DynamicEntityPropertyId);
        ...
        {
            List<DynamicEntityPropertyValue> allValues =
                await DynamicEntityPropertyValueStore.GetValuesAsync(entityFullName, entityId);
            List<DynamicEntityPropertyValue> returnList = new List<DynamicEntityPropertyValue>();

            foreach (DynamicEntityPropertyValue value in allValues)
            {
                DynamicEntityProperty dynamicEntityProperty =
                    await _dynamicEntityPropertyManager.GetAsync(value.DynamicEntityPropertyId);
        ...
        {
            DynamicProperty dynamicProperty = _dynamicPropertyManager.Get(propertyName);
        ...
        public async Task<List<DynamicEntityPropertyValue>> GetValuesAsync(string entityFullName, string entityId,
            string propertyName)
        {
            DynamicProperty dynamicProperty = await _dynamicPropertyManager.GetAsync(propertyName);
            if (dynamicProperty == null)
            {
                throw new EntityNotFoundException($"There is no DynamicProperty with propertyName: \"{propertyName}\"");
            }
