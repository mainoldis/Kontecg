...
namespace Kontecg.DynamicEntityProperties
{
    public class DynamicPropertyValueManager : IDynamicPropertyValueManager, ITransientDependency
    {
        ...
        {
            DynamicPropertyValue val = DynamicPropertyValueStore.Get(id);
        ...
        {
            DynamicPropertyValue val = await DynamicPropertyValueStore.GetAsync(id);
        ...
        {
            DynamicPropertyValue val = Get(id);
        ...
        {
            DynamicPropertyValue val = await GetAsync(id);
            if (val != null) //Get checks permission, no need to check it again
            {
