...
using Kontecg.Text.Formatting;

namespace Kontecg.Text
{
    /// <summary>
    ...
    public class FormattedStringValueExtracter
    {
            ...
            char? splitformatCharacter = null)
        {
            ...
            List<FormatStringToken> formatTokens = TokenizeFormat(format, splitformatCharacter);
            ...
            ExtractionResult result = new ExtractionResult(false);

            for (int i = 0; i < formatTokens.Count; i++)
            {
                FormatStringToken currentToken = formatTokens[i];
                FormatStringToken previousToken = i > 0 ? formatTokens[i - 1] : null;
                ...
                if (currentToken.Type == FormatStringTokenType.ConstantText)
                {
                    ...
                    {
                        int matchIndex = str.IndexOf(currentToken.Text, stringComparison);
                        if (matchIndex >= 0)
                        {
            ...
            FormatStringToken lastToken = formatTokens.Last();
        ...
        {
            ExtractionResult result = new FormattedStringValueExtracter().Extract(str, format, ignoreCase);
            if (!result.IsMatch)
            {
        ...
        private List<FormatStringToken> TokenizeFormat(string originalFormat, char? splitformatCharacter = null)
        {
            ...
            List<FormatStringToken> result = new List<FormatStringToken>();
            string[] formats = originalFormat.Split(splitformatCharacter.Value);
