using System;
using System.Collections.Generic;
using System.Threading.Tasks;

namespace Kontecg.Webhooks
{
    public interface IWebhookSubscriptionManager
    {
        /// <summary>
        ///     Returns subscription for given id.
        /// </summary>
        /// <param name="id">Unique identifier of <see cref="WebhookSubscriptionInfo" /></param>
        Task<WebhookSubscription> GetAsync(Guid id);

        /// <summary>
        ///     Returns subscription for given id.
        /// </summary>
        /// <param name="id">Unique identifier of <see cref="WebhookSubscriptionInfo" /></param>
        WebhookSubscription Get(Guid id);

        /// <summary>
        ///     Returns all subscriptions of company
        /// </summary>
        /// <param name="companyId">
        ///     Target company id.
        /// </param>
        Task<List<WebhookSubscription>> GetAllSubscriptionsAsync(int? companyId);

        /// <summary>
        ///     Returns all subscriptions of company
        /// </summary>
        /// <param name="companyId">
        ///     Target company id.
        /// </param>
        List<WebhookSubscription> GetAllSubscriptions(int? companyId);

        /// <summary>
        ///     Returns all subscriptions for given webhook.
        /// </summary>
        /// <param name="webhookName">
        ///     <see cref="WebhookDefinition.Name" />
        /// </param>
        /// <param name="companyId">
        ///     Target company id.
        /// </param>
        Task<List<WebhookSubscription>> GetAllSubscriptionsIfFeaturesGrantedAsync(int? companyId, string webhookName);

        /// <summary>
        ///     Returns all subscriptions for given webhook.
        /// </summary>
        /// <param name="companyId">
        ///     Target company id.
        /// </param>
        /// <param name="webhookName">
        ///     <see cref="WebhookDefinition.Name" />
        /// </param>
        List<WebhookSubscription> GetAllSubscriptionsIfFeaturesGranted(int? companyId, string webhookName);

        /// <summary>
        ///     Returns all subscriptions of company
        /// </summary>
        /// <returns></returns>
        Task<List<WebhookSubscription>> GetAllSubscriptionsOfCompanysAsync(int?[] companyIds);

        /// <summary>
        ///     Returns all subscriptions of company
        /// </summary>
        /// <param name="companyIds">
        ///     Target company id(s).
        /// </param>
        List<WebhookSubscription> GetAllSubscriptionsOfCompanys(int?[] companyIds);

        /// <summary>
        ///     Returns all subscriptions for given webhook.
        /// </summary>
        /// <param name="webhookName">
        ///     <see cref="WebhookDefinition.Name" />
        /// </param>
        /// <param name="companyIds">
        ///     Target company id(s).
        /// </param>
        Task<List<WebhookSubscription>> GetAllSubscriptionsOfCompanysIfFeaturesGrantedAsync(int?[] companyIds,
            string webhookName);

        /// <summary>
        ///     Returns all subscriptions for given webhook.
        /// </summary>
        /// <param name="companyIds">
        ///     Target company id(s).
        /// </param>
        /// <param name="webhookName">
        ///     <see cref="WebhookDefinition.Name" />
        /// </param>
        List<WebhookSubscription> GetAllSubscriptionsOfCompanysIfFeaturesGranted(int?[] companyIds, string webhookName);

        /// <summary>
        ///     Checks if company subscribed for a webhook. (Checks if webhook features are granted)
        /// </summary>
        /// <param name="companyId">
        ///     Target company id(s).
        /// </param>
        /// <param name="webhookName">
        ///     <see cref="WebhookDefinition.Name" />
        /// </param>
        Task<bool> IsSubscribedAsync(int? companyId, string webhookName);

        /// <summary>
        ///     Checks if company subscribed for a webhook. (Checks if webhook features are granted)
        /// </summary>
        /// <param name="companyId">
        ///     Target company id(s).
        /// </param>
        /// <param name="webhookName">
        ///     <see cref="WebhookDefinition.Name" />
        /// </param>
        bool IsSubscribed(int? companyId, string webhookName);

        /// <summary>
        ///     If id is the default(Guid) adds new subscription, else updates current one. (Checks if webhook features are
        ///     granted)
        /// </summary>
        Task AddOrUpdateSubscriptionAsync(WebhookSubscription webhookSubscription);

        /// <summary>
        ///     If id is the default(Guid) adds it, else updates it. (Checks if webhook features are granted)
        /// </summary>
        void AddOrUpdateSubscription(WebhookSubscription webhookSubscription);

        /// <summary>
        ///     Activates/Deactivates given webhook subscription
        /// </summary>
        /// <param name="id">unique identifier of <see cref="WebhookSubscriptionInfo" /></param>
        /// <param name="active">IsActive</param>
        Task ActivateWebhookSubscriptionAsync(Guid id, bool active);

        /// <summary>
        ///     Delete given webhook subscription.
        /// </summary>
        /// <param name="id">unique identifier of <see cref="WebhookSubscriptionInfo" /></param>
        Task DeleteSubscriptionAsync(Guid id);

        /// <summary>
        ///     Delete given webhook subscription.
        /// </summary>
        /// <param name="id">unique identifier of <see cref="WebhookSubscriptionInfo" /></param>
        void DeleteSubscription(Guid id);
    }
}
