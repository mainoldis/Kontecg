...
namespace Kontecg.Runtime.Validation.Interception
{
    ...
    public class MethodInvocationValidator : ITransientDependency
    {
        ...
        public void Validate()
        {
            ...
            foreach (IShouldNormalize objectToBeNormalized in ObjectsToBeNormalized)
            {
        ...
        protected virtual void CheckInitialized()
        {
            if (Method == null)
            {
                throw new KontecgException("This object has not been initialized. Call Initialize method first.");
            }
        }
        ...
        protected virtual bool IsValidationDisabled()
        {
            ...
            return ReflectionHelper
                .GetSingleAttributeOfMemberOrDeclaringTypeOrDefault<DisableValidationAttribute>(Method) != null;
        }
        ...
        protected virtual void ValidateMethodParameter(ParameterInfo parameterInfo, object parameterValue)
        {
            if (parameterValue == null)
            {
                ...
                {
                    ValidationErrors.Add(new ValidationResult(parameterInfo.Name + " is null!",
        ...
        protected virtual void ValidateObjectRecursively(object validatingObject, int currentDepth)
        {
            ...
            {
                foreach (object item in (IEnumerable) validatingObject)
                {
        ...
        {
            foreach (Type validatorType in _configuration.Validators)
            {
                if (ShouldValidateUsingValidator(validatingObject, validatorType))
                {
                    using IDisposableDependencyObjectWrapper<IMethodParameterValidator> validator =
                        _iocResolver.ResolveAsDisposable<IMethodParameterValidator>(validatorType);
                    IReadOnlyList<ValidationResult> validationResults = validator.Object.Validate(validatingObject);
        ...
        protected virtual bool ShouldMakeDeepValidation(object validatingObject)
        {
            ...
            Type validatingObjectType = validatingObject.GetType();
