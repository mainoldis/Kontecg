...
namespace Kontecg.Resources.Embedded
{
    public class EmbeddedResourceManager : IEmbeddedResourceManager, ISingletonDependency
    {
        ...
        {
            string encodedPath = EmbeddedResourcePathHelper.EncodeAsResourcesPath(fullPath);
        ...
        {
            Dictionary<string, EmbeddedResourceItem> resources =
                new Dictionary<string, EmbeddedResourceItem>(StringComparer.OrdinalIgnoreCase);

            foreach (EmbeddedResourceSet source in _configuration.Sources)
            {
