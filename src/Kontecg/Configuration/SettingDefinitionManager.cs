...
namespace Kontecg.Configuration
{
    ...
    internal class SettingDefinitionManager : ISettingDefinitionManager, ISingletonDependency
    {
        ...
        {
            if (!_settings.TryGetValue(name, out SettingDefinition settingDefinition))
            {
                throw new KontecgException("There is no setting defined with name: " + name);
            }
        ...
        public void Initialize()
        {
            ...
            foreach (Type providerType in _settingsConfiguration.Providers)
            {
                using IDisposableDependencyObjectWrapper<SettingProvider> provider = CreateProvider(providerType);
                foreach (SettingDefinition settings in provider.Object.GetSettingDefinitions(context))
                {
