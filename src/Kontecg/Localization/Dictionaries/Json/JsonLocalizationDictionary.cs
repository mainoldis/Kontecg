...
namespace Kontecg.Localization.Dictionaries.Json
{
    /// <summary>
    ...
    public class JsonLocalizationDictionary : LocalizationDictionary
    {
        ...
        public static JsonLocalizationDictionary BuildFromJsonString(string jsonString)
        {
            ...
            {
                throw new KontecgException("Can not parse json string. " + ex.Message);
            }

            string cultureCode = jsonFile.Culture;
            if (string.IsNullOrEmpty(cultureCode))
            {
                throw new KontecgException("Culture is empty in language json file.");
            }

            JsonLocalizationDictionary dictionary =
                new JsonLocalizationDictionary(CultureInfo.GetCultureInfo(cultureCode));
            List<string> dublicateNames = new List<string>();
            foreach (KeyValuePair<string, string> item in jsonFile.Texts)
            {
                if (string.IsNullOrEmpty(item.Key))
                {
                    throw new KontecgException("The key is empty in given json string.");
                }
