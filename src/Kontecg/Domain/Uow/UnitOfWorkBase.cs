...
namespace Kontecg.Domain.Uow
{
    ...
    public abstract class UnitOfWorkBase : IUnitOfWork
    {
        ...
        /// <summary>
        /// Reference to current KONTECG session.
        /// </summary>
        public IKontecgSession KontecgSession { protected get; set; }
        ...
        public void Begin(UnitOfWorkOptions options)
        {
            ...
            SetCompanyId(KontecgSession.CompanyId, false);
        ...
        public virtual IDisposable SetCompanyId(int? companyId)
        {
            return SetCompanyId(companyId, true);
        }

        public virtual IDisposable SetCompanyId(int? companyId, bool switchMustHaveCompanyEnableDisable)
        {
            var oldCompanyId = _companyId;
            _companyId = companyId;


            IDisposable mustHaveCompanyEnableChange;
            if (switchMustHaveCompanyEnableDisable)
            {
                mustHaveCompanyEnableChange = companyId == null
                    ? DisableFilter(KontecgDataFilters.MustHaveCompany)
                    : EnableFilter(KontecgDataFilters.MustHaveCompany);
            }
            else
            {
                mustHaveCompanyEnableChange = NullDisposable.Instance;
            }

            var mayHaveCompanyChange = SetFilterParameter(KontecgDataFilters.MayHaveCompany, KontecgDataFilters.Parameters.CompanyId, companyId);
            var mustHaveCompanyChange = SetFilterParameter(KontecgDataFilters.MustHaveCompany, KontecgDataFilters.Parameters.CompanyId, companyId ?? 0);

            return new DisposeAction(() =>
            {
                mayHaveCompanyChange.Dispose();
                mustHaveCompanyChange.Dispose();
                mustHaveCompanyEnableChange.Dispose();
                _companyId = oldCompanyId;
            });
        }
        ...
        private void PreventMultipleBegin()
        {
            ...
            {
                throw new KontecgException("This unit of work has started before. Can not call Start method more than once.");
            }
        ...
        private void PreventMultipleComplete()
        {
            ...
            {
                throw new KontecgException("Complete is called before!");
            }
        ...
        private void SetFilters(List<DataFilterConfiguration> filterOverrides)
        {
            ...
            {
                ChangeFilterIsEnabledIfNotOverrided(filterOverrides, KontecgDataFilters.MustHaveCompany, false);
            }
        }
        ...
        private DataFilterConfiguration GetFilter(string filterName)
        {
            ...
            if (filter == null)
            {
                throw new KontecgException("Unknown filter name: " + filterName + ". Be sure this filter is registered before.");
            }
