...
{
    public class MarkdownPreprocessor : IMarkdownPreprocessor, ISingletonDependency
    {
        /// <summary>
        ...
        private const string
            AutoLinkPreventionMarker = "\x1AP"; // temporarily replaces "://" where auto-linking shouldn't happen
        ...
        private static readonly Regex LinkDef = new($@"
                        ^[ ]{{0,{TabWidth - 1}}}\[([^\[\]]+)\]:  # id = $1
                          [ ]*
                          \n?                   # maybe *one* newline
                          [ ]*
        ...
        private static readonly Regex AnchorInline = new($@"
        ...
        private static readonly Regex AnchorRefShortcut = new(@"
            (                               # wrap whole match in $1
              \[
        ...
        private static readonly Regex ImagesRef = new(@"
                    (               # wrap whole match in $1
                    !\[
                        (.*?)       # alt text = $2
                    \]
        ...
        private static readonly Regex ImagesInline = new(string.Format(@"
              (                     # wrap whole match in $1
                !\[
                    (.*?)           # alt text = $2
                \]
        ...
        private static readonly Regex ListNested = new("^" + WholeList,
            RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);

        private static readonly Regex ListTopLevel = new(@"(?:(?<=\n\n)|\A\n?)" + WholeList,
            RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace | RegexOptions.Compiled);

        private static readonly Regex CodeBlock = new(string.Format(@"
                    (?:\n\n|\A\n?)
        ...
        private static readonly Regex Bold = new(@"(\*\*|__) (?=\S) (.+?[*_]*) (?<=\S) \1",
            RegexOptions.IgnorePatternWhitespace | RegexOptions.Singleline | RegexOptions.Compiled);

        private static readonly Regex SemiStrictBold = new(
        ...
        private static readonly Regex Italic = new(@"(\*|_) (?=\S) (.+?) (?<=\S) \1",
            RegexOptions.IgnorePatternWhitespace | RegexOptions.Singleline | RegexOptions.Compiled);

        private static readonly Regex SemiStrictItalic = new(
        ...
        private static readonly Regex AutolinkBare = new(
            @"(<|="")?\b(https?|ftp)(://" + CharInsideUrl + "*" + CharEndingUrl + ")(?=$|\\W)",
            RegexOptions.IgnoreCase | RegexOptions.Compiled);

        private static string _nestedBracketsPattern;

        private readonly Regex _amps = new("&(?!((#[0-9]+)|(#[xX][a-fA-F0-9]+)|([a-zA-Z][a-zA-Z0-9]*));)",
            RegexOptions.ExplicitCapture | RegexOptions.Compiled);

        private readonly Regex _anchorRef = new($@"
            (                               # wrap whole match in $1
                \[
                    ({GetNestedBracketsPattern()})                   # link text = $2
                \]
        ...
        private readonly Regex _angles = new(@"<(?![A-Za-z/?\$!])",
            RegexOptions.ExplicitCapture | RegexOptions.Compiled);

        private readonly Regex _autoEmailBare = new(@"(<|="")?(?:mailto:)?([-.\w]+\@[-a-z0-9]+(\.[-a-z0-9]+)*\.[a-z]+)",
            RegexOptions.IgnoreCase | RegexOptions.Compiled);

        private readonly Regex _backslashEscapes;

        // Table of hash value for backslash escaped characters
        private readonly Dictionary<string, string> _backslashEscapeTable = new();

        private readonly Regex _blockquote = new(@"
            (                           # Wrap whole match in $1
                (
        ...
        private readonly Regex _blocksHtml =
            new(GetBlockPattern(), RegexOptions.Multiline | RegexOptions.IgnorePatternWhitespace);

        private readonly Regex _codeEncoder = new(@"&|<|>|\\|\*|_|\{|\}|\[|\]", RegexOptions.Compiled);

        private readonly IMarkdownDefaultOptions _defaultOptions;

        private readonly Regex _endCharRegex = new(CharEndingUrl, RegexOptions.IgnoreCase | RegexOptions.Compiled);

        // Table of hash values for escaped characters
        private readonly Dictionary<string, string> _escapeTable = new();

        private readonly Regex _htmlBlockHash = new("\x1AH\\d+H", RegexOptions.Compiled);
        private readonly Dictionary<string, string> _htmlBlocks = new();

        private readonly Regex _htmlTokens = new(@"
            (<!--(?:|(?:[^>-]|-[^>])(?:[^-]|-[^-])*)-->)|        # match <!-- foo -->
            (<\?.*?\?>)|                 # match <?foo?> " +
                                                 RepeatString(@" 
            (<[A-Za-z\/!$](?:[^<>]|", NestDepth - 1) + @"
        ...
        private readonly Dictionary<string, string> _invertedEscapeTable = new();
        private readonly Regex _leadingWhitespace = new("^[ ]*", RegexOptions.Compiled);

        private readonly Regex _linkEmail = new(@"<
                      (?:mailto:)?
                      (
                        [-.\w]+
                        \@
        ...
        private readonly Regex _newlinesLeadingTrailing = new(@"^\n+|\n+\z", RegexOptions.Compiled);
        private readonly Regex _newlinesMultiple = new(@"\n{2,}", RegexOptions.Compiled);

        private readonly Regex _outDent = new("^[ ]{1," + TabWidth + "}",
...
#pragma warning disable RCS1190 // Join string expressions.
        private readonly Regex _unescapes = new("\x1A" + "E\\d+E", RegexOptions.Compiled);
#pragma warning restore RCS1190 // Join string expressions.
        ...
        public MarkdownPreprocessor(IMarkdownDefaultOptions defaultOptions)
        {
        ...
        public string Transform(string text, MarkdownOptions options)
        {
        ...
        private string FormParagraphs(string text, bool unhash = true, bool createParagraphs = true)
        {
            ...
            for (int i = 0; i < grafs.Length; i++)
            {
                ...
                else
                {
                    ...
                    grafs[i] = _leadingWhitespace.Replace(RunSpanGamut(grafs[i]), createParagraphs ? "<p>" : "") +
                               (createParagraphs ? "</p>" : "");
                }
            }
        ...
        /// <summary>
        ///     Reusable pattern to match balanced [brackets]. See Friendl's
        ...
        private string DoAnchors(string text)
        {
            ...
            //  or [link test](/foo)
            return AnchorRefShortcut.Replace(text, AnchorRefShortcutEvaluator);
        }
        ...
        private string DoImages(string text)
        {
            ...
            return ImagesInline.Replace(text, ImageInlineEvaluator);
        }
        ...
        private string ImageTag(string url, string altText, string title)
        {
            ...
            url = AttributeSafeUrl(url);
            string result = $"<img src=\"{url}\" alt=\"{altText}\"";
            if (!string.IsNullOrEmpty(title))
            {
                title = AttributeEncode(EscapeBoldItalic(title));
                result += $" title=\"{title}\"";
            ...
            result += _options.EmptyElementSuffix;
            return result;
        }
        ...
        private string DoHeaders(string text)
        {
            ...
            return HeaderAtx.Replace(text, AtxHeaderEvaluator);
        }
        ...
        {
            return HorizontalRules.Replace(text, "<hr" + _options.EmptyElementSuffix + "\n");
        }
        ...
        private string DoLists(string text)
        {
            ...
            return _listLevel > 0 ? ListNested.Replace(text, ListEvaluator) : ListTopLevel.Replace(text, ListEvaluator);
        }
        ...
        private string ListEvaluator(Match match)
        {
            ...
            string result = ProcessListItems(list, listType == "ul" ? MarkerUl : MarkerOl);
        ...
        private string ProcessListItems(string list, string marker)
        {
            ...
            string ListItemEvaluator(Match match)
            {
                ...
                bool loose = containsDoubleNewline || lastItemHadADoubleNewline;
        ...
        {
            return CodeBlock.Replace(text, CodeBlockEvaluator);
        }
        ...
        private string DoItalicsAndBold(string text)
        {
            ...
            if (_options.StrictBoldItalic.HasValue && _options.StrictBoldItalic.Value)
            {
                if (_options.AsteriskIntraWordEmphasis.HasValue && _options.AsteriskIntraWordEmphasis.Value)
                {
        ...
        {
            return _options.AutoNewlines.HasValue && _options.AutoNewlines.Value
                ? Regex.Replace(text, @"\n", $"<br{_options.EmptyElementSuffix}\n")
                : Regex.Replace(text, @" {2,}\n", $"<br{_options.EmptyElementSuffix}\n");
        }

        /// <summary>
        ///     Turn Markdown > quoted blocks into HTML blockquotes blocks
        /// </summary>
        ...
        private string HandleTrailingParens(Match match)
        {
        ...
        private string EmailBareLinkEvaluator(Match match)
        {
        ...
        {
            if (_options.AutoHyperlink.HasValue && _options.AutoHyperlink.Value)
                // fix-up arbitrary URLs by adding Markdown < > so they get linked as well
            ...
            if (_options.LinkEmails.HasValue && _options.LinkEmails.Value)
        ...
        private string AttributeEncode(string s)
        {
            return s.Replace(">", "&gt;").Replace("<", "&lt;").Replace("\"", "&quot;").Replace("'", "&#39;");
        }

        private string AttributeSafeUrl(string s)
        {
            s = AttributeEncode(s);
            foreach (char c in "*_:()[]")
            {
        ...
        private string EscapeSpecialCharsWithinTagAttributes(string text)
        {
            ...
            foreach (Token token in tokens)
            {
                ...
                if (token.Type == TokenType.Tag)
                {
                    ...
                    if (_options.AutoHyperlink.HasValue && _options.AutoHyperlink.Value && value.StartsWith("<!")
                       ) // escape slashes in comments to prevent auto-linking there -- https://meta.stackexchange.com/questions/95987/html-comment-containing-url-breaks-if-followed-by-another-html-comment
                    {
                        value = value.Replace("/", _escapeTable["/"]);
                    }
