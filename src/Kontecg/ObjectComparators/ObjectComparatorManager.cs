...
namespace Kontecg.ObjectComparators
{
    public class ObjectComparatorManager : IObjectComparatorManager, ITransientDependency
    {
        ...
        {
            foreach (IObjectComparator objectComparator in _objectComparators)
            {
        ...
        public bool Compare<TBaseType, TEnumCompareType>(TBaseType baseObject, TBaseType compareObject,
            TEnumCompareType compareType)
            where TEnumCompareType : Enum
        {
            return Compare(baseObject, compareObject, compareType.ToString());
        }
        ...
        public bool Compare<TBaseType, TEnumCompareType>(TBaseType baseObject,
            ObjectComparatorCondition<TBaseType, TEnumCompareType> condition) where TEnumCompareType : Enum
        {
            foreach (IObjectComparator objectComparator in _objectComparators)
            {
